...
```text
Release ONDEWO NLU Typescript Client ${ONDEWO_NLU_VERSION}
```

So the heading of a new entry must read exactly `## Release ONDEWO NLU Typescript Client <version>`. **This wording is
...
```bash
grep -c '^## Release ONDEWO NLU Typescript Client ' RELEASE.md     # must be >= 1 for your new version
```
...
This package publishes as **`@ondewo/nlu-client-typescript`**, which is not always the repository name — the JS client
...
```bash
npm view @ondewo/nlu-client-typescript versions --json
```
